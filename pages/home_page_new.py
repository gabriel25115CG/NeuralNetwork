...
class HomePage(tk.Frame):
    ...
    def update_model_display(self):
        ...
        for model in self.saved_models:
            model_name = model.get('name', 'Sans nom').lower()
        ...
        if filtered_models:
            for model in filtered_models:
                ...
                name = model.get('name', 'Sans nom')
                date_str = model.get('creation_date', 'N/A')
                if date_str != 'N/A':
                    try:
                ...
                layers_config = model.get('layers_config', [])
                if layers_config:
                ...
                # Taille des fichiers
                total_size = model.get('metadata_size', 0) + model.get('network_size', 0)
                size_str = format_file_size(total_size)
                
                # Dataset utilisé
                dataset = model.get('dataset_file', 'N/A')
...
INFORMATIONS DU MODÈLE : {model.get('name', 'Sans nom')}
{'='*50}

📊 ARCHITECTURE
- Couches : {model.get('layers_config', 'N/A')}
- Paramètres totaux : {model.get('total_params', 'N/A')}
- Fonction d'activation : {model.get('activation', 'N/A')}

📈 ENTRAÎNEMENT  
- Dataset : {model.get('dataset_file', 'N/A')}
- Nombre d'échantillons : {model.get('n_samples', 'N/A')}
- Variables cibles : {model.get('target_column', 'N/A')}
- Loss finale : {model.get('final_loss', 'N/A')}

📁 FICHIERS
- Taille totale : {format_file_size(model.get('metadata_size', 0) + model.get('network_size', 0))}
- Date de création : {model.get('creation_date', 'N/A')}
- Nom de fichier : {model.get('filename', 'N/A')}

📝 DESCRIPTION
{model.get('description', 'Aucune description disponible')}
        """.strip())
    ...
    def update_statistics(self):
        ...
        total_size = sum(model.get('metadata_size', 0) + model.get('network_size', 0) for model in self.saved_models)
        ...
        if messagebox.askyesno("Confirmation", message):
            try:
                ...
                for filename in selected_filenames:
                    if self.model_persistence.delete_model(filename):
                        deleted_count += 1
